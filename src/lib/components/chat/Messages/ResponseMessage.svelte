				...
				{#if !edit}
					...
					>
						{#if message.done || siblings.length > 1}
							...
							{#if message.done}
								{#if !readOnly && polaproConfig.showEditMessage}
								...
								{#if polaproConfig.showCopyMessage}
								...
								{#if polaproConfig.showReadAloudMessage && !readOnly && ($user?.role === 'admin' || ($user?.permissions?.chat?.tts ?? true))}
								...
								{#if !readOnly}
									...
									{#if polaproConfig.showContinueResponseMessage && isLastMessage && ($user?.role === 'admin' || ($user?.permissions?.chat?.continue_response ?? true))}
