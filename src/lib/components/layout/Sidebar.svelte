...
<SurveyModal bind:show={showSurveyModal} />
	...
	>
		...
		>
			...
			<div class="-mt-[0.5px]">
				...
				<div>
					<Tooltip content="Feedback-Fragebogen" placement="right">
						<button
							class=" cursor-pointer flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition group"
							on:click={(e) => {
								e.stopImmediatePropagation();
							...
							draggable="false"
							aria-label="Feedback-Fragebogen"
						>
							<div class=" self-center flex items-center justify-center size-9">
				...
				{#if polaproConfig.showWorkspace && ($user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools)}
					<div class="">
				...
				<div class="pb-1.5">
					...
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-sm font-primary">Feedback</div>
							</div>
						</button>
					</div>
